...
function [B, mdl, LL] = fitglm_wrapper_crossval(varargin)
...
if crossval
    ...
    for run = runs
        for sub = subjects
            ...
            B(sub,:,run) = this_mdl.Coefficients.Estimate';
            LL(sub,run)  = get_LL_logregression(data,B(sub,:,run),dep_var_name,predictors,interactions,use,sub,run);
            mdl(sub,run) = {this_mdl};
        end
    end
end
