...
function [B, mdl, LL] = fitglm_wrapper_no_z(varargin)
...
%     pred_z      = glm_wrapper_z(use_vals); % Do z transf
    ...
    % get interactions
    if do_ints
        ints    = glm_get_interactions(predictors,interactions,pred);
        pred  = [pred;ints];
    end

    % Do regression
%     [B(sub,:), DEV(sub), STAT(sub)] = fitglm(pred_z',use_dep',glm_options{1},glm_options{2:end});%'binomial','link','probit');
    this_mdl = fitglm(pred',use_dep',glm_options{1},glm_options{2:end});%'binomial','link','probit');
